...
import com.dronexbe.manager.DroneCommentManager;
...
public class DroneCommentController {
	...
	public ResponseEntity<DroneComment> addDroneComment(@RequestBody DroneComment droneComment) {
		...
		// the commentId field was null on input and should have been set by hibernate
		// on a successful add
	...
	// -------------------Count All (any) Comments for a Drone----------
	@RequestMapping(value = "/droneCommentsCount/{id}", method = RequestMethod.GET)
	public ResponseEntity<Long> howManyDroneCommentByDroneId(@PathVariable("id") Long id) {
		return new ResponseEntity<Long>(droneCommentManager.howManyDroneComments(id), HttpStatus.OK);
	}
